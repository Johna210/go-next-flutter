...
func main() {
	...
	app := fx.New(
		core.Module,
		modules.Modules,
		fx.Populate(&migrator.SchemaManager, &migrator.Config, &migrator.Logger),
		fx.NopLogger,
	)
	...
	env := flag.String("env", "local", "Environment: local, development, production")
	...
	case "generate":
		...
		migrator.generateMigration(*name, *env, *modules)
	case "apply":
		migrator.applyMigrations(*env)
	case "status":
		migrator.checkStatus(*env)
	default:
		migrator.Logger.Fatal("Invalid action. Use: list, generate, apply, or status")
	}
}
